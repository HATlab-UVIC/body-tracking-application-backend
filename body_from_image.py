...
#All code outside the class is for setup of openpose --------------------------------------------------------------------------
tracking_img_pth = '../../../examples/media/'
...
# Flags
new_path = tracking_img_pth + file_name
...
print('Open Pose Start Up: {}'.format(end_time - start_time))
...
class body_from_image:
    ...
    def find_points(save_folder, file_name):
        ...
        try:
            ...
            # Process Image
            datum = op.Datum()
            imageToProcess = cv2.imread(save_folder + file_name) #args[0].image_path
            datum.cvInputData = imageToProcess
            opWrapper.emplaceAndPop(op.VectorDatum([datum]))
            os.remove(save_folder + file_name)
            end_time = datetime.now()
            #print('Open Pose Image: {}'.format(end_time - start_time))
